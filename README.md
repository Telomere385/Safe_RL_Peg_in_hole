...
- **Phase 1.5 (当前)** — 把 peg/hole 作为**视觉 only** 的子节点挂在左/右夹
  ...
  `get_preinsert_frames()`。MDP 语义 (obs / reward / is_absorbing) 暂时仍是
  phase 1 的 reaching，phase 1.5 只改资产和"读"接口，不改训练语义，便于
  commit-by-commit 验收。
...
- **Commit 3 (规划中)**: 误差 helper `_compute_preinsert_errors()` 还没实装。
  规划里它会返回 `pos_err = ||peg_tip - preinsert_target||` 和
  `axis_err = 1 + dot(peg_axis, hole_axis) ∈ [0,2]` (0 = 轴反平行 = 理想对齐)，
  与 `pose_success_mask`。落地前仍不进 reward/obs/is_absorbing。
...
- `train_sac.py` / `eval_sac.py` 的 eval episode 数要与 `num_envs` 对齐
  (默认直接取 `num_envs`)；若尾批不足，`VectorCore.evaluate` 会把 inactive env
  teleport away，渲染里就会看到"机器人飞天"。
